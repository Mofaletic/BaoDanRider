...
开发中，我负责确定玩法方向、判断体验问题和验收结果，使用 AI 协助拆分任务、实现 C++ 功能、运行检查和同步文档。
...
三项问题分别涉及关卡碰撞、镜头和操控。为了避免混在同一轮修改，我将它整理成下面的执行 Prompt：
...
整理执行任务时补齐了五项信息：**复现条件、修改顺序、保护范围、验收标准、验证方式**。
...
1. **先写玩家体验，再写功能名称**：先说明“玩家应该感觉到什么”，防止实现只停留在表面按钮或变量。
...
7. **通过后立即记录并冻结**：同步策划案和项目状态，防止后续修改覆盖已经确认的结论。

<details>
<summary><strong>可复用的游戏原型任务 Prompt 模板</strong></summary>
...
每轮迭代都记录目标、修改范围和验证结果；确认通过的内容写入策划案和项目状态，供下一轮继续使用。
...
开发分工：我负责玩法设计、试玩反馈和验收，AI 用于代码实现、测试编排和文档维护。
